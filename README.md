...
| RC2 | Right stick up/down / elevator | Plane elevator on CH9; transition elevator plus slight front ESC boost on CH1/CH3; hover pitch mix on front/rear ESCs and mounts with CH9 elevator centered |
...
| RC7 | Control mode switch | Low = RC mode and slow-blink lights; high = PID mode and fast-blink lights |
| RC8 | Trainer / PID gain | Scales throttle above idle, servo/mount travel, stick-driven ESC boosts, PID target bias, and PID correction output from 25% at low to 100% at high |

RC-derived ESC output is capped at `1600us`, which is 60% of the normal `1000us` to `2000us` ESC command range. The remaining 40% is reserved for the Pico's future PID stabilization output, not direct joystick authority. Servo and mount outputs use an expanded `500us` to `2500us` range so full receiver stick travel produces wider servo travel. RC8 acts as a trainer/PID gain: low maps to 25% authority and high maps to full 100% authority. The gain scales throttle above idle, servo deflection from center, mount travel from center, rear mount yaw/aileron mix, hover pitch mix, hover side-to-side ESC boost, PID target bias, and PID correction output; RC5 still selects the flight mode and RC7 selects RC/PID control mode. RC input values use a `6us` deadband to reduce servo jitter from tiny receiver pulse noise. RC5 maps the switch positions to calibrated mount postures: low/plane sends `1580us`, middle/transition sends `1915us`, and high/hover sends `2250us` before RC8 gain scaling. The rear motors are physically set opposite the front motors, so the same mode pulse creates the opposite front/rear posture.

In `flightmode1`, RC2 controls the elevator servo on CH9, RC4 controls the rudder servo, and RC1 controls the rear engine mounts differentially like ailerons. In `flightmode2`, RC2 still controls the elevator and also slightly boosts the front ESCs on CH1 and CH3; RC4 controls the rudder servo and both RC1 and RC4 contribute to rear mount aileron movement on CH6 and CH8. In `flightmode3`, CH9 elevator and CH10 rudder stay centered, RC4 controls rear mount yaw tilt, RC1 commands side-to-side hover movement by slightly increasing the opposing-side ESC pair, and RC2 commands hover pitch: stick up boosts the rear ESCs on CH5 and CH7 while nudging the rear mounts upward, and stick back boosts the front ESCs on CH1 and CH3 while slightly lowering the front mounts. If the receiver signal is missing, ESC outputs go to `1000us`, servos center at `1500us`, RC/PID mode falls back to RC mode, and the flight mode falls back to `flightmode1`.

In RC mode, the remote behaves as the direct command source. In PID mode, the Pico captures the current sensor state when RC7 goes high, then treats remote stick movement as a target-direction bias instead of a replacement for stabilization. Hover mode tries to hold the current world state using level pitch/roll, heading, and bottom-ToF altitude when available; stick commands bias that hold and keep the hover feedforward mixes active, so stick-right still slightly boosts the left front/rear ESCs while PID stabilizes around that movement. Plane mode holds the captured trajectory through attitude and heading corrections while stick commands bias the commanded trajectory. Transition mode blends hover and flight behavior with half-strength corrections. The current sensor stack does not provide GPS or optical-flow X/Y position, so PID mode cannot truly lock horizontal X/Y world position yet; it can only stabilize attitude, heading, and altitude while applying directional movement commands.

The light controller is now driven by mode-change pulses on CH12 instead of direct RC7 passthrough. The controller starts solid when powered; one off/on pulse selects the slow blink pattern, a second off/on pulse selects fast blink, and a third cycles back to solid. RC mode targets slow blink. PID mode targets fast blink.

Rear mount aileron movement is limited before mixing into CH6 and CH8 so roll inputs do not swing the rear motor mounts through their full mechanical range. Plane mode uses 50% rear aileron authority, and transition mode uses 25% rear aileron authority.

In transition and hover, rear mount yaw is capped at the flight-mode mount setting, so turning inputs can pull a rear mount toward flight position but not past it. In transition mode, rudder/yaw input is not added to the aileron differential mix: left rudder pulls the right rear mount toward flight position, and right rudder pulls the left rear mount toward flight position.
